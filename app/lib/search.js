const min2phase = (function () {
      ...
      Math.max(
        USE_CONJ_PRUN
          ? getPruningMax(
              ...
              TwistFlipPrun,
              ((this.twistc >> 3) << 11) |
                FlipS2RF[this.flipc ^ (this.twistc & 7)],
            )
          : 0,
        USE_TWIST_FLIP_PRUN
          ? getPruningMax(
              ...
              TwistFlipPrun,
              (this.twist << 11) |
                FlipS2RF[(this.flip << 3) | (this.fsym ^ this.tsym)],
            )
          : 0,
      ),
    );
  ...
  Search.prototype.verify = function (facelets) {
    ...
    if (
      (getNParity(getNPermFull(this.cc.ea, 12, true), 12) ^
  ...
  Search.prototype.initPhase2 = function (phase2Cubie) {
    ...
    const prun = Math.max(
      getPruningMax(
        ...
        EPermCCombPPrun,
        p2edge * N_COMB +
          CCombPConj[Perm2CombP[p2corn] & 0xff][SymMultInv[p2esym][p2csym]],
      ),
      getPruningMax(
        MCPermPrunMax,
        MCPermPrun,
  ...
  Search.prototype.appendSolMove = function (curMove) {
    ...
    if (axisCur == axisLast) {
      var pow =
        ((curMove % 3) + (this.moveSol[this.moveSol.length - 1] % 3) + 1) % 4;
      if (pow == 3) {
        this.moveSol.pop();
      } else {
    ...
    if (
      ...
      var pow =
        ((curMove % 3) + (this.moveSol[this.moveSol.length - 2] % 3) + 1) % 4;
      if (pow == 3) {
    ...
    for (let m = 0; m < 10; m++) {
      ...
      if (
        getPruningMax(
          ...
          EPermCCombPPrun,
          (edgei >> 4) * N_COMB +
            CCombPConj[Perm2CombP[corni >> 4] & 0xff][
  ...
  function initBasic() {
    ...
    for (var i = 0; i < N_COMB; i++) {
      ...
      for (var j = 0; j < N_MOVES2; j++) {
        ...
        CCombPMove[i][j] =
          d.getCComb() + 70 * (((P2_PARITY_MOVE >> j) & 1) ^ ~~(i / 70));
      }
      for (var j = 0; j < 16; j++) {
    ...
    while (depth < SEARCH_DEPTH) {
      ...
      for (var i = 0; i < N_SIZE; i++, val >>= 4) {
        ...
        for (let m = 0; m < N_MOVES; m++) {
          ...
          for (
            let j = 1, symState = SymState[symx];
            (symState >>= 1) != 0;
            j++
          ) {
            if ((symState & 1) != 1) {
