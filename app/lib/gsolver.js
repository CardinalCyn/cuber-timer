...
const gsolver = (function () {
  ...
  // 	const solv = new mathlib.gSolver(
  ...
  const rubiksCube = (function () {
    ...
    // 	}
    // 	oriSelect = $('<select>');
    // 	for (let i = 0; i < cubeOris.length; i++) {
  ...
  const sq1Cube = (function () {
    ...
    // 	solv1 = solv1 || new mathlib.gSolver(['0Aa0Aa0Aa0Aa|Aa0Aa0Aa0Aa0', '0Aa0Aa0Aa0Aa|0Aa0Aa0Aa0Aa', 'Aa0Aa0Aa0Aa0|Aa0Aa0Aa0Aa0', 'Aa0Aa0Aa0Aa0|0Aa0Aa0Aa0Aa'], sq1Move, moves);
    // 	solv2 = solv2 || new mathlib.gSolver(['0Aa0Aa0Aa0Aa|Bb1Bb1Bb1Bb1', '0Aa0Aa0Aa0Aa|1Bb1Bb1Bb1Bb', 'Aa0Aa0Aa0Aa0|Bb1Bb1Bb1Bb1', 'Aa0Aa0Aa0Aa0|1Bb1Bb1Bb1Bb'], sq1Move, moves);
    // 	curScramble = [];
  ...
  const skewbCube = (function () {
    ...
    // 		solv ||
    // 		new mathlib.gSolver(
    // 			['?L?L??B?B?UUUUU?R?R???F?F?????', '?F?F??L?L?UUUUU?B?B???R?R?????', '?R?R??F?F?UUUUU?L?L???B?B?????', '?B?B??R?R?UUUUU?F?F???L?L?????'],
    // 			skewbMove,
    // 			appendSuffix(
    // 				{
    // 					R: 0x0,
  ...
  // 			sol = [];
  // 			var sol1;
  // 			out: for (let depth = 0; depth < 99; depth++) {
  // 				for (let j = 0; j < 3; j++) {
  ...
  // const slideCube = (function () {
  // 	function slideMove(state, m) {
  // 		const blank = state.indexOf('-');
  // 		let x = blank >> 2;
  // 		let y = blank & 3;
  ...
  // 		const ret = [];
  // 		const mirrorPerm = [0, 4, 8, 12, 1, 5, 9, 13, 2, 6, 10, 14, 3, 7, 11, 15];
  // 		for (let i = 0; i < 16; i++) {
  ...
  // 	const solv1 = new mathlib.gSolver(fixBlank('0123????????????'), slideMove, moves);

  // 	const solv2 = new mathlib.gSolver(fixBlank('$$$$4???8???c???'), slideMove, moves);

  // 	const solv3 = new mathlib.gSolver(['$$$$$567$9ab$de-'], slideMove, moves);
  ...
  // 		const ret = [];
  // 		const moveRef = midx == 1 ? 'VH' : 'HV';
  // 		const symbol = settings.indexOf('a') == -1 ? ['DR', 'UL'] : ['\uFFEC\uFFEB', '\uFFEA\uFFE9'];
  // 		const isBlankMove = settings.indexOf('m') != -1;
  // 		const compress = settings.indexOf('p') != -1;
  // 		const pos = [-1, -1];
  // 		for (var i = 0; i < sol.length; i++) {
  // 			const val = ~~sol[i][1];
  // 			const m = moveRef.indexOf(sol[i][0]);
  ...
  // 			var move = ret[i];
  // 			const axis = symbol[isBlankMove != move[1] > 0 ? 0 : 1][move[0]];
  // 			let pow = Math.abs(move[1]);
  // 			ret[i] = [];
  // 			if (compress) {
  // 				ret[i].push(axis + pow);
  // 			} else {
  ...
  // 				}
  // 			}
  // 			const sol2 = solv2.search(stateInit('01234???8???c??-', perm[midx]).replace(/[0123]/g, '$'), 0);
  // 			sol = sol.concat(sol2);
  // 		} else if (size == 3) {
  // 			const perm8 = randPerm(3);
  // 			const slide8 = [5, 6, 7, 9, 10, 11, 13, 14, 15];
  // 			for (let i = 0; i < 16; i++) {
  ...
  // 			perm = [perm];
  // 		}
  // 		const sol3 = solv3.search(stateInit('0123456789abcde-', perm[midx]).replace(/[012348c]/g, '$'), 0);
  // 		sol = sol.concat(sol3);
  ...
  // 	}
  // 	scrMgr.reg(['15prp', '15prap', '15prmp'], getScramble.bind(null, 4))(['8prp', '8prap', '8prmp'], getScramble.bind(null, 3));
  // })();

  // const general333Solver = execMain(function () {
  // 	let isSolving = 0; //0 - idle, 1 - single, 2 - batch
  // 	const selectPre = $('<select style="font-size:0.75em;">');
  // 	const selectMode = $('<select style="font-size:0.75em;">');
  // 	const solveButton = $('<input type="button" value="Solve!" style="font-size:0.75em;">');
  // 	const resultText = $('<textarea wrap=off rows="8" cols="30" style="font-size:0.75em; display:none;">');
  // 	const resultSpan = $('<span>');
  // 	const subsetInfo = $('<span style="display:block;">');
  // 	let resultArr = [];
  // 	const presets = {
  ...
  // 	};
  // 	let canvas, ctx;
  // 	let solvedState = mathlib.SOLVED_FACELET;
  // 	const colors = {
  // 		U: '#fff',
  // 		R: '#f00',
  // 		F: '#0d0',
  ...
  // 	let selColor = '-';
  // 	const width = 30;
  // 	const offxs = [1, 2, 1, 1, 0, 3, 7.25];
  // 	const offys = [0, 1, 1, 2, 1, 1, 0.5];
  // 	const selXYs = [
  // 		[-0.7, -0.7, 0.7, 0.7],
  // 		[-0.7, 0.7, 0.7, -0.7],
  // 	];
  // 	const offw = 3.3;
  // 	const stateRE = /^[URFDLBXYZ-]{54}$/;

  // 	function procClick(e) {
  // 		clearSolve();
  // 		const rect = canvas[0].getBoundingClientRect();
  // 		const cordX = ((e.offsetX / width) * canvas[0].width) / rect.width;
  // 		const cordY = ((e.offsetY / width) * canvas[0].height) / rect.height;

  // 		for (let face = 0; face < 6; face++) {
  // 			if (cordX >= offxs[face] * offw && cordX <= offxs[face] * offw + 3 && cordY >= offys[face] * offw && cordY <= offys[face] * offw + 3) {
  // 				var i = ~~(cordX - offxs[face] * offw);
  // 				var j = ~~(cordY - offys[face] * offw);
  // 				const tmp = solvedState.split('');
  ...
  // 		const imgSize = kernel.getProp('imgSize') / 48;
  ...
  // 		const sizeH = params[0][0] * params[1][0] + params[0][1] * params[1][1];
  // 		const indexH = 43252003274489856000 / sizeH;
  // 		subsetInfo.html(
  ...
  // 			return;
  // 		}
  // 		const ret = curSolver[1].searchNext(1000, 1000);
  // 		let curStatus = '';
  ...
  // 			resultText.hide();
  // 		}
  // 		const startState = stateInit(rubiksCube.move, solvedState);
  // 		const ret = curSolver[1].search(startState, 0, 0);
  // 		startTime = +new Date();
  // 		resultArr = [];
  // 		searchThread();
  // 	}
  ...
  // 				$('<tr>').append($('<td style="padding:0;">').append(selectPre.unbind('change').change(selectChange), selectMode, solveButton.unbind('click').click(toggleSolve))),
  ...
  // 		}
  // 		fdiv.empty();
  // 		const span = $('<span class="sol"/>');
  // 		const scramble = tools.getCurScramble();
